...
import { BasePokemon, PokemonDetail, GymLeaderInfo, GymLeaderStrategy, SupportedLanguage } from './types';
import { getPokemonDetailsById } from './services/pokemonService';
import { getGymLeaderCounterStrategy } from './services/genaiService';
...
const App: React.FC = () => {
  ...
  const [gymLeaderStrategies, setGymLeaderStrategies] = useState<Map<string, GymLeaderStrategy | null | 'loading' | 'error'>>(new Map());
  ...
  useEffect(() => {
    const fetchGymLeaderStrategies = async () => {
      if (currentView === 'gymLeaders') {
        const newStrategies = new Map(gymLeaderStrategies);
        let needsUpdate = false;

        for (const leader of KALOS_GYM_LEADERS) {
          if (!newStrategies.has(leader.id) || newStrategies.get(leader.id) === null) {
            newStrategies.set(leader.id, 'loading');
            needsUpdate = true;
          }
        }
        if (needsUpdate) {
            setGymLeaderStrategies(new Map(newStrategies)); 
        }
        
        for (const leader of KALOS_GYM_LEADERS) {
            if (newStrategies.get(leader.id) === 'loading') { 
                 try {
                    const leaderDisplayName = t(leader.gym_leader_key, currentLanguage);
                    const specialtyDisplayName = t(leader.specialty_display_key, currentLanguage);
                    const strategy = await getGymLeaderCounterStrategy(leaderDisplayName, specialtyDisplayName, currentLanguage);
                    newStrategies.set(leader.id, strategy);
                 } catch (error) {
                    console.error(`Falha ao buscar estratégia para ${leader.gym_leader_key}:`, error);
                    newStrategies.set(leader.id, 'error');
                 } finally {
                    setGymLeaderStrategies(new Map(newStrategies));
                 }
            }
    ...
    fetchGymLeaderStrategies();
  }, [currentView, currentLanguage, gymLeaderStrategies]); 
  ...
  const handleChangeLanguage = (lang: SupportedLanguage) => {
    ...
    if (currentView === 'gymLeaders') {
      setGymLeaderStrategies(new Map()); 
    }
  };
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 max-w-7xl mx-auto">
        {KALOS_GYM_LEADERS.map((gymLeader) => ( 
          <GymLeaderCard 
            ...
            strategy={gymLeaderStrategies.get(gymLeader.id)}
            currentLanguage={currentLanguage}
          />
        ))}
      </div>
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 md:p-6 lg:p-8">
      <header className="mb-6 sm:mb-8 pt-3 sm:pt-4 px-2">
        ...
        <div className="flex flex-col items-center sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
          ...
          <div className="hidden sm:block sm:w-20 md:w-24 lg:w-32 flex-shrink-0 sm:order-1"></div> {/* Adjusted width slightly for larger title */}
          ...
          <div className="text-center w-full sm:w-auto sm:flex-grow sm:order-2">
            <div className="inline-flex items-center justify-center space-x-1 xs:space-x-2 sm:space-x-3">
              <img 
                ...
                className="w-8 h-8 xs:w-9 xs:h-9 sm:w-11 sm:h-11 md:w-14 md:h-14 lg:w-16 lg:h-16 flex-shrink-0"
              />
              <h1 
                className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-sky-400 tracking-tight truncate"
              >
      ...
      <footer className="text-center mt-12 sm:mt-16 py-6 sm:py-8 border-t border-slate-700">
        <p className="text-slate-500 text-xs sm:text-sm">
          ...
          {" "}{t("Enrichment by Gemini API.", currentLanguage)}
        </p>
