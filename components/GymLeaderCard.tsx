...
import { GymLeaderInfo, GymLeaderStrategy, SupportedLanguage } from '../types';
import { POKEMON_TYPE_COLORS, capitalizeForDisplay } from '../constants';
...
interface GymLeaderCardProps {
  ...
  strategy?: GymLeaderStrategy | null | 'loading' | 'error';
  currentLanguage: SupportedLanguage;
}

const GymLeaderCard: React.FC<GymLeaderCardProps> = ({ gymInfo, isDefeated, onToggleDefeated, strategy, currentLanguage }) => {
  ...
  const renderStrategy = () => {
    if (strategy === 'loading') {
      return (
        <div className="mt-2 text-xs text-slate-400 flex items-center justify-center">
          <LoadingSpinner size="sm" />
          <span className="ml-2">{t("Analyzing strategy...", currentLanguage)}</span>
        </div>
      );
    }
    if (strategy === 'error' || !strategy) {
      return <p className="mt-2 text-xs text-red-400">{t("Counter strategy unavailable.", currentLanguage)}</p>;
    }
    
    if (typeof strategy === 'object' && strategy !== null && 'recommendedTypes' in strategy && 'tacticalTip' in strategy) {
       const strat = strategy as GymLeaderStrategy; 
       return (
        <div className="mt-2 text-left w-full">
          <h4 className="text-[0.7rem] sm:text-xs font-semibold text-sky-300 mb-1">{t("Counter Strategy:", currentLanguage)}</h4>
          {strat.recommendedTypes && strat.recommendedTypes.length > 0 && (
            <div className="mb-1.5">
              <p className="text-[0.65rem] sm:text-[0.7rem] text-slate-400 mb-0.5">{t("Recommended Types:", currentLanguage)}</p>
              <div className="flex flex-wrap gap-1 justify-center sm:justify-start">
                {strat.recommendedTypes.map(type => { 
                  const recTypeColors = POKEMON_TYPE_COLORS[type.toLowerCase()] || POKEMON_TYPE_COLORS['normal'];
                  return (
                    <span
                      key={type}
                      className={`px-1.5 py-0.5 text-[0.6rem] leading-tight sm:text-[0.65rem] rounded-full font-medium ${recTypeColors.background} ${recTypeColors.text} ${recTypeColors.border ? `border ${recTypeColors.border}` : ''} shadow-sm`}
                    >
                      {getTranslatedType(type, currentLanguage)}
                    </span>
                  );
                })}
              </div>
          ...
          {strat.tacticalTip && ( 
             <div>
                <p className="text-[0.65rem] sm:text-[0.7rem] text-slate-400 mb-0.5">{t("Tactical Tip:", currentLanguage)}</p>
                <p className="text-[0.65rem] sm:text-xs text-slate-300 leading-snug">{strat.tacticalTip}</p>
             </div>
          )}
        </div>
      );
    }
    return <p className="mt-2 text-xs text-red-400">{t("Counter strategy data is invalid.", currentLanguage)}</p>;
  };
